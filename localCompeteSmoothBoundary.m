function localCompeteSmoothBoundary( data, positions, reviewMode )
...
while true
    row_boundary = [];
    col_boundary = [];
    prevSeg = data.I.tempSeg;
    for bn=1:length(row_sel)
        bounds = data.I.tempSeg(row_sel(bn)-1:row_sel(bn)+1, col_sel(bn)-1:col_sel(bn)+1);
        if  all(bounds==bounds(1,1)) % BUG => works on any arbitrary boundary, i.e. not restricted to the boundary between the tissues we are interested in, as it should
            continue;
        end
        row_boundary = [row_boundary row_sel(bn)];%coordinates of the boundary points
        col_boundary = [col_boundary col_sel(bn)];
    end
    
    for tr=1:length(row_boundary)
        window = data.I.lab(row_boundary(tr)-2:row_boundary(tr)+2,col_boundary(tr)-2:col_boundary(tr)+2,:);
        ...
        if selectedRegion(row_boundary(tr),col_boundary(tr)) == 1
            data.I.tempSeg(row_boundary(tr),col_boundary(tr)) = uniqueRegValues(ii);
        end
    end
    if prevSeg==data.I.tempSeg
        break;
    end
end
