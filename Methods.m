...
switch command_str
    ...
    case 'prepScript Female'
        for k=1:3
            ...
            %data = competeSelect(data, 0, 3, [28 220],false,false);     % blood vessel, Tendon
            %data = competeSelectSmooth(data, 0, 2, [20 136],false,false);    % Aorta, Fat
            ...
            data = competeSelectSmooth(data, 0, 2, [2 204],false,false);    % Air, SkinDry           
            data = competeSelectSmooth(data, 0, 3, [136 4],false,false);    % fat, colon
            data = competeSelectSmooth(data, 0, 3, [136 120],false,false);    % fat, Uterus
            ...
            data = competeSelectSmooth(data, 0, 3, [164 136],false,false);    % smallIntestine, Fat                      
    ...
    case 'addBoneCortical'
        set(data.buttons(4),'Enable','off');
        set(data.buttons(5),'Enable','off');
        data.I.tempSeg2 = data.I.tempSeg;
        fprintf('Select region to add bone cortical to...\n');
        k=1; % draw boundary for competition
        hFreehands(k) = imfreehand(data.handles.axes(1));
        position{k} = wait(hFreehands(k));
%         fprintf('Select first region''s mean...\n');
...
%         end
        ...
        set(data.buttons(4),'Enable','on');
        set(data.buttons(5),'Enable','on');
        addBoneCortical(data, position, true);
        
    otherwise
        errordlg('Invalid command_str for Methods.','Something is not right: Invalid command_str for Methods.')
end
end