function localCompeteSmooth( data, positions, reviewMode )
...
for tr=1:length(row_sel)
    window = data.I.lab(row_sel(tr)-2:row_sel(tr)+2,col_sel(tr)-2:col_sel(tr)+2,:);
    ...
    meanl=mean2(weights.*tl)/mean2(weights); % normalization
