function localCompeteSmoothOriginal( data, positions, reviewMode )
...
for k=2:length(positions)
    ...
    %reg = roipoly(localLAB,col1,row1);
...
tally(length(data.I.tempSeg(:,1)),length(data.I.tempSeg(1,:)),length(uniqueRegValues)) = 0;
dif(1:length(uniqueRegValues))=0;
window=zeros(wsize,wsize,3,'uint8');

for tr=1:length(row_sel)
    window = data.I.lab(row_sel(tr):row_sel(tr)+wsize-1,col_sel(tr):col_sel(tr)+wsize-1,:);
    ...
    for tn=1:length(uniqueRegValues)
        ...
        for tk=1:length(regMeans(:,1))
            if regValues(tk)==uniqueRegValues(tn)
                dif(tn)=dif(tn)+(regMeans(tk,1)-meanl).^2 + (regMeans(tk,2)-meana).^2 + (regMeans(tk,3)-meanb).^2;
                %cc=cc+1;
            end
        end
        %dif(tn)=dif(tn)/cc;
    ...
    [~, ii]=min(dif);
    for ti=row_sel(tr):row_sel(tr)+wsize-1
        for tj=col_sel(tr):col_sel(tr)+wsize-1
            if selectedRegion(ti,tj) == 1
                tally(ti,tj,ii) = tally(ti,tj,ii) +1;
            end
        end
    end
    
end
for ar = 1:length(row_sel)
    for ai=row_sel(ar):row_sel(ar)+wsize-1
        for aj=col_sel(ar):col_sel(ar)+wsize-1
            if max(tally(ai,aj,:)) == 0
                continue;
            end
            if selectedRegion(ai,aj) == 1
                [~, adet] = max(tally(ai,aj,:));
                data.I.tempSeg(ai,aj) = uniqueRegValues(adet);
            end
        end
    end
end
